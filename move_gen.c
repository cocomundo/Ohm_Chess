...
int move_gen( int *board, int depth, int alpha, int beta, bool black_or_white,
    ...
    bool short_castle_w, bool short_castle_b, MOVE *best_move, int material)
{
    ...
    int copy_piece = 0; /* the captured piece */
    int copy_pos = 0; /* the previous square */
            ...
            switch (board[square]) {
                case 1: /* Pawn */
                    if(board[square+dir_wP[1]] == 0) {
                        if(square+dir_wP[1] < 30) {
                            ...
                            for(int i = 2; i < 6; i++)
                            {
                                ...
                                eval = move_gen(board, depth-1, alpha, beta, false, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL,material+material_list[i]-material_list[1]);
                        ...
                        if(square > 80 && (board[square+dir_wP[2]] == 0)) {
                            ...
                            /* after white move -> black move -> false */
                            eval = move_gen(board, depth-1, alpha, beta, false, square+dir_wP[2], long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material);
                            if((depth == MAXDEPTH) && (eval > max) && (best_move != NULL)){
                                best_move->start_pos = square;
                        ...
                        /* after white move -> black move -> false */
                        eval = move_gen(board, depth-1, alpha, beta, false, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material);
                        if((depth == MAXDEPTH) && (eval > max) && (best_move != NULL)){
                            best_move->start_pos = square;
                            best_move->end_pos = square+dir_wP[1];
                            best_move->special = 0;
                    ...
                    for(int y = 3; y < 5; y++) {
                        if(board[square+dir_wP[y]] > 10) {
                            ...
                            if(square+dir_wP[y] < 30) {
                                ...
                                {
                                    board[square] = i;
                                    eval = move_gen(board, depth-1, alpha, beta, false, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL,material+material_list[i]-material_list[1]-material_list[copy_piece]);
                                    if((depth == MAXDEPTH) && (eval > max) && (best_move != NULL)){
                                        best_move->start_pos = square;
                                ...
                                eval = move_gen(board, depth-1, alpha, beta, false, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material-material_list[copy_piece]);
                                if((depth == MAXDEPTH) && (eval > max) && (best_move != NULL)){
                                    best_move->start_pos = square;
                                    best_move->end_pos = square+dir_wP[y];
                                    best_move->special = 0;
                    ...
                    if( (enpassant_square - square) == 1 ||
                        ...
                        eval = move_gen(board, depth-1, alpha, beta, false, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material-material_list[11]);
                        if((depth == MAXDEPTH) && (eval > max) && (best_move != NULL)){
                            best_move->start_pos = square;
                        ...
                        while(board[copy_pos+dir_R[y]] == 0 || board[copy_pos+dir_R[y]] > 10) {
                            ...
                            if (board[copy_pos+dir_R[y]] > 10) {
                                ...
                                if(long_castle_w ==1 && square == 91){
                                    eval = move_gen(board, depth-1, alpha, beta, false, false, false, long_castle_b, short_castle_w, short_castle_b, NULL, material-material_list[copy_piece]);
                                }else if(short_castle_w==1 && square == 98){
                                    eval = move_gen(board, depth-1, alpha, beta, false, false, long_castle_w, long_castle_b, false, short_castle_b, NULL, material-material_list[copy_piece]);
                                }else{
                                    eval = move_gen(board, depth-1, alpha, beta, false, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material-material_list[copy_piece]);
                                } 
                            ...
                            /* If the Rook moves from it's starting square, castles to this side becomes illegal*/
                            if(square == 91){
                                eval = move_gen(board, depth-1, alpha, beta, false, false, false, long_castle_b, short_castle_w, short_castle_b, NULL, material);
                            }else if(square == 98){
                                eval = move_gen(board, depth-1, alpha, beta, false, false, long_castle_w, long_castle_b, false, short_castle_b, NULL, material);
                            }else{
                                eval = move_gen(board, depth-1, alpha, beta, false, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material);
                            } 
                ...
                case 6:/* King */
                    for(int y = 1; y < dir_Q_Ki[0]; y++) {
                        if(board[square+dir_Q_Ki[y]] == 0 || board[square+dir_Q_Ki[y]] > 10) {
                            ...
                            eval = move_gen(board, depth-1, alpha, beta, false, false, false, long_castle_b, false, short_castle_b, NULL, material-material_list[copy_piece]);
                            if((depth == MAXDEPTH) && (eval > max) && (best_move != NULL)){
                                best_move->start_pos = square;
                                best_move->end_pos = copy_pos+dir_Q_Ki[y];
                                best_move->special = 0;
                    ...
                    if(long_castle_w==1 && square==95 && board[94]==0 && board[93]==0 && board[92]==0 && board[91]==4)
                    {
                        ...
                        eval = move_gen(board, depth-1, alpha, beta, false, false, false, long_castle_b, false, short_castle_b, NULL, material);
                        if((depth == MAXDEPTH) && (eval > max) && (best_move != NULL)){
                ...
                case 11:/* Pawn */
                    if(board[square+dir_bP[1]] == 0) {
                        if(square+dir_bP[1] > 90) {
                            ...
                            {
                                board[square] = i;
                                eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material-material_list[11]+material_list[i]);
                        ...
                        if((square < 40) && (board[square+dir_bP[2]] == 0)) {
                            ...
                            eval = move_gen(board, depth-1, alpha, beta, true, square+dir_bP[2], long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material); 
                            if((depth == MAXDEPTH) && (eval < min) && (best_move != NULL)){
                                best_move->start_pos = square;
                        ...
                        /* after black move -> white move -> true */
                        eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material);
                        if((depth == MAXDEPTH) && (eval < min) && (best_move != NULL)){
                            best_move->start_pos = square;
                            best_move->end_pos = square+dir_bP[1];
                            best_move->special = 0;
                        ...
                        if(board[square+dir_bP[y]] < 10 && board[square+dir_bP[y]] > 0) {
                            if(square+dir_bP[y] > 90) {
                                ...
                                for(int i = 12; i < 16; i++)
                                {
                                    board[square] = i;
                                    /* after black move -> white move -> true */
                                    eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material+material_list[i]-material_list[11]-material_list[copy_piece]); 
                                    if((depth == MAXDEPTH) && (eval < min) && (best_move != NULL)){
                                        best_move->start_pos = square;
                                ...
                                /* after white move -> black move -> false */
                                eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material-material_list[copy_piece]); 
                                if((depth == MAXDEPTH) && (eval < min) && (best_move != NULL)){
                                    best_move->start_pos = square;
                                    best_move->end_pos = square+dir_bP[y];
                                    best_move->special = 0;
                    ...
                    if( (enpassant_square - square) == 1 ||
                        ...
                        /* after white move -> black move -> false */
                        eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material-material_list[1]);
                        if((depth == MAXDEPTH) && (eval < min) && (best_move != NULL)){
                            best_move->start_pos = square;
                            best_move->end_pos = enpassant_square + 10;
                            best_move->special = 1;
                            ...
                            while(board[copy_pos+dir_R[y]] == 0 || (board[copy_pos+dir_R[y]] < 10 && board[copy_pos+dir_R[y]] > 0)) {
                                if (board[copy_pos+dir_R[y]] < 10 && board[copy_pos+dir_R[y]] > 0) {
                                    ...
                                    if(long_castle_b==1 && square == 21){
                                        eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, false, short_castle_w, short_castle_b, NULL, material-material_list[copy_piece]);
                                    }else if(short_castle_b==1 &&  square == 28){
                                        eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, long_castle_b, short_castle_w, false, NULL, material-material_list[copy_piece]);
                                    }else{
                                        eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material-material_list[copy_piece]);
                                    } 
                                ...
                                /* If the Rook moves from it's starting square, castles to this side becomes illegal*/
                                if(long_castle_b==1 && square == 21){
                                    eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, false, short_castle_w, short_castle_b, NULL, material);
                                }else if(short_castle_b==1 && square == 28){
                                    eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, long_castle_b, short_castle_w, false, NULL, material);
                                }else{
                                    eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, long_castle_b, short_castle_w, short_castle_b, NULL, material);
                                } 
                ...
                case 16:/* KING */
                        for(int y = 1; y < dir_Q_Ki[0]; y++) {
                            if(board[square+dir_Q_Ki[y]] == 0 || ((board[square+dir_Q_Ki[y]] < 10) && board[square+dir_Q_Ki[y]] > 0)) {
                                ...
                                eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, false, short_castle_w, false, NULL, material-material_list[copy_piece]);
                                if((depth == MAXDEPTH) && (eval < min) && (best_move != NULL)){
                                    best_move->start_pos = square;
                        ...
                        if(long_castle_b==1 &&square==25 && board[24]==0 && board[23]==0 && board[22]==0 && board[21]==14)
                        {
                            ...
                            eval = move_gen(board, depth-1, alpha, beta, true, false, long_castle_w, false, short_castle_w, false, NULL, material);
                            if((depth == MAXDEPTH) && (eval < min) && (best_move != NULL)){
                                best_move->start_pos = 0;
                                best_move->end_pos = 0;
                                best_move->special = 3;
